...
typedef struct PR {
	...
	char fileName[MAX_FILENAME_SIZE]; //ptr unusable in shared memory
...
void catch_signal(int theSignal){
	
  if (theSignal == SIGUSR1) {
	
	printf("\nKill signal received.\n");
	printf("Detaching shared memory segment & closing semaphores.\n");
...
void printClient(){
	...
	sleep(1); // Allow for server to create shared seg
