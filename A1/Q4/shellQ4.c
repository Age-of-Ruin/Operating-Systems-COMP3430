...
char *swapVarsWithinToken(int posnCount, char *token, int varPosn[], char **inputVars, char **outputVars, int varCount){
...
void checkAndSwapVars(char **tokenArray, int tokenCount, char **inputVars, char **outputVars, int varCount){
	...
	int varPosn[100]; // hold variable position within string
	int posnCount; // index the array of positions
	...
	// check if variable to be converted was entered
		for (i = 0; i < tokenCount; i++){
			...
			if(strstr(tokenArray[i], "$")){
				...
				tokenArray[i] = swapVarsWithinToken(posnCount, tokenArray[i], varPosn, inputVars, outputVars, varCount);			
